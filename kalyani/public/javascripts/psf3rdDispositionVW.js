...
function loadLeadBasedOnUserLocation(lead,tag){
	...
	console.log("tag is "+tag);
	 ...
	 var urlLink="/CRE/getLeadNamesbyLeadId/" +leadId+ "/" +wyzuserId+"/" +moduletype+ "";
	 console.log("lead urlLink : "+urlLink);
	    $.ajax({
	        url: urlLink

	    }).done(function (leadName) {
	    	if(leadName.length!=0){
	    	$('#'+tag).empty();
	          var dropdown = document.getElementById(tag);
	          
	          for(var i=0;i<leadName.length;i++){
	         
	                  dropdown[dropdown.length] = new Option(leadName[i].name,leadName[i].id);
...
function loadLeadBasedOnUserLocationSelected(locais,lead,tag){
	...
	var locaisName=document.getElementById(locais).value;
	 ...
	 var urlLink="/CRE/getLeadNamesbyLeadLocation/" +leadId+ "/" +wyzuserId+"/" +moduletype+ "/"+locaisName;
	 console.log("lead urlLink : "+urlLink);
	    $.ajax({
	        url: urlLink
