$(document).ready(function () {
    ...
    $("input[name='psfinteraction.isContacted']").click(function () {
        ...
        if (PSFtest == "No") {
            ...
            $("#PSFYesTalkFirstFlow").hide();
            $("#PSFYesTalk").hide();
            $("#SaveInCompleteServeyDiv").hide();
            $("#remarksDivDisplay").show();
            $("input:radio[name='psfinteraction.PSFDispositon']").each(function (i) {
                this.checked = false;
            });
    ...
    $("input[name='listingForm.maindisposition']").click(function () {
        PSFtest = $(this).val();
        if (PSFtest == "ResolvedStatus") {
    ...
    $("#RootCauseOfProblem").click(function () {
        PSFtest = $(this).val();
        if ($(this).is(':checked')) {
            $("#rootCauseCheck").show();
    ...
    $("input[name$='psfinteraction.PSFDispositon']").click(function () {
        console.log("dispo id is " + this.id);
        // alert(" varPSFYes "+varPSFYes);

        if (this.id == "ReWorkIdSch") {


            $("#reworkModeData").val("Schedule a revisit");

        } else if (this.id == "ReWorkId") {

            $("#reworkModeData").val("Home visit");
        } else {
            $("#reworkModeData").val("0");
        }

        varPSFYes = $(this).val();
        if (varPSFYes == "PSF_Yes" || varPSFYes == "Call Me Later" || varPSFYes == "Not Interested" || varPSFYes == "No Resolution - Closed" || varPSFYes == "Re-Work" || varPSFYes == "Resolved" || varPSFYes == "Escaltion to CEO") {
            psfNamaskar();
        }
        else {
            if (varPSFYes != "NoOther") {
                $('.OtherComments').val('');
            }
        }
    });
    ...
    function psfNamaskar() {
        ...
        if (varPSFYes != "") {
            ...
            clearPrv(varPSFYes, 'radio');
        }

    }


    function clearPrv(varPSFYes, typeOfAction) {

        $('#workshop_id_Data').val('');
        $('#schDateId').val('');

        $('#dateandtimepicker').val('');
        $('#resolvedById').val('');
...
//< !--------------------- MMS jquery------------------------------------------------------>
     ...
     * $("#LeadDivAlreadyService").show(); $('.PSFUpsell
...
$('#noresolutionSubmit').click(function () {
    ...
    var rootdata = $('#rootCauseId').val();

    console.log("rootdata : " + rootdata);

    if (chkincSubmit == 0 || rootdata == 0 || rootdata=='') {
        Lobibox.notify('warning', {
            continueDelayOnInactiveTab: true,
...
$('#reWorkSubmit').click(function () {
    ...
    if (workVal == '' || schDVal == '' || rootdata == 0 || rootdata == '' || visitType=='') {
        ...
        if (schDVal == '') {
            Lobibox.notify('warning', {
                msg: 'Please enter Appointment Date and Time.'
            });
            return false;
        }
        ...
        if (rootdata == '') {
            Lobibox.notify('warning', {
                msg: 'Please enter RC Issue Type.'
            });
            return false;
        }
        else {
...
$('#reSolvedSubmit').click(function () {
    ...
    if (dealerCode == "BRIDGEWAYMOTORS") {
        if (resModVal == '') {
            Lobibox.notify('warning', {
                msg: 'Please enter Action Taken.'
            });
            return false;
        }
        if (benVal == '') {
            Lobibox.notify('warning', {
                msg: 'Please Enter Exact Nature of complaint.'
            });
            return false;
        }
    }
    else {
    if (dtVal == '' || resByVal == '' || resModVal == '' || benVal == '') {
        if (resByVal == '') {
            Lobibox.notify('warning', {
                msg: 'Please enter Attended By.'
            });
            return false;
        }
        if (dtVal == '') {
            Lobibox.notify('warning', {
                msg: 'Please enter Attended on.'
            });
            return false;
        }
        if (resModVal == '') {
...
$('#noAfterresolution').click(function () {
    var rootdata = $('#rootCauseId').val();

    if (rootdata == 0 || rootdata=='') {
        Lobibox.notify('warning', {
            msg: 'Please enter RC Issue Type.'
        });
        return false;
...
function bindSubRoot() {
    var PSFtest = $('#rootCauseId').val();
    var rootSubCause = $('#rootSubCauseHd');

    var rootSubvalue = $("#rootsubtype").val();
    $(rootSubCause).val($("#rootsubtype").val());

    //if (PSFtest == "Delay") {
    //    var subcause = $("#DealyId").val();
       
    //} else if (PSFtest == "Service Quality") {

    //    var subcause = $("#DealyId").val();
    //    $(rootSubCause).val($("#rootServiceQId").val());
    //}
    //else if (PSFtest == "SA Related") {

    //    $(rootSubCause).val($("#rootSARealatedId").val());
    //}
    //else if (PSFtest == "Service Initiation") {

    //    $(rootSubCause).val($("#rootServiceInitiatorId").val());
    //}
    //else if (PSFtest == "Washing Cleaning") {
    //    $(rootSubCause).val($("#rootwashingCleaningId").val());
    //}
    //else if (PSFtest == "Others") {
    //    $(rootSubCause).val($("#rootothersId").val());
    //}
}
